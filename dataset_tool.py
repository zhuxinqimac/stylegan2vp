...
def create_mnist(tfrecord_dir, mnist_dir):
    ...
    images = np.pad(images, [(0, 0), (0, 0), (2, 2), (2, 2)],
                    'constant',
                    constant_values=0)
...
def create_test_mnist(tfrecord_dir, mnist_dir):
    print('Loading MNIST from "%s"' % mnist_dir)
    import gzip
    with gzip.open(os.path.join(mnist_dir, 't10k-images-idx3-ubyte.gz'),
                   'rb') as file:
        images = np.frombuffer(file.read(), np.uint8, offset=16)
    with gzip.open(os.path.join(mnist_dir, 't10k-labels-idx1-ubyte.gz'),
                   'rb') as file:
        labels = np.frombuffer(file.read(), np.uint8, offset=8)
    images = images.reshape(-1, 1, 28, 28)
    images = np.pad(images, [(0, 0), (0, 0), (2, 2), (2, 2)],
...
def create_lsun_wide(tfrecord_dir,
    ...
    with lmdb.open(lmdb_dir, readonly=True).begin(write=False) as txn:
        ...
        if max_images is None:
            max_images = total_images
        with TFRecordExporter(tfrecord_dir, max_images,
            ...
            for idx, (_key, value) in enumerate(txn.cursor()):
                try:
                    ...
                    print('\r%d / %d => %d ' %
                          (idx + 1, total_images, tfr.cur_images),
                          end='')
...
def create_from_dsprites_npz(tfrecord_dir, dsprites_filename, shuffle,
                             shape_only, sample_color):
    ...
    with TFRecordExporter(tfrecord_dir, images.shape[0]) as tfr:
        ...
        for idx in range(order.size):
            ...
            if sample_color:
                color = np.repeat(np.repeat(np.random.uniform(
                    0.5, 1, [3, 1, 1]),
                                            img.shape[1],
                                            axis=1),
                                  img.shape[2],
...
def execute_cmdline(argv):
    ...
    p = add_command('create_test_mnist', 'Create testdataset for MNIST.',
                    'create_test_mnist datasets/mnist ~/downloads/mnist')
    p.add_argument('tfrecord_dir', help='New dataset directory to be created')
    p.add_argument('mnist_dir', help='Directory containing test MNIST')
    ...
    p.add_argument('--height',
                   help='Output height (default: 384)',
                   type=int,
                   default=384)
    p.add_argument('--max_images',
                   help='Maximum number of images (default: none)',
    ...
    p.add_argument('--cx',
                   help='Center X coordinate (default: 89)',
                   type=int,
                   default=89)
    p.add_argument('--cy',
    ...
    p = add_command(
        'create_from_dsprites_npz', 'Create dataset from a dsprites_filename.',
    ...
    p.add_argument('--shuffle',
                   ...
                   type=int,
                   default=0)
    p.add_argument('--shape_only',
                   help='Use only shape as label',
                   type=int,
                   default=1)
    p.add_argument('--sample_color',
